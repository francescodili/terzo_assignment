...
def save_boxes(tracks, frame, save_path, confs):
    with open(save_path, 'a') as file:
        for track, conf in zip(tracks, confs):
            ...
            line = f"{frame}, {track.track_id}, {bb_left}, {bb_top}, {bb_width}, {bb_height}, {conf}, -1, -1, -1\n"
            file.write(line)
    ...
    for dir in os.listdir(data_path):
        for video_dir in os.listdir(os.path.join(data_path, dir)):
            if video_dir == 'img1':
                ...
                for frame_path in sorted(os.listdir(os.path.join(data_path, dir, video_dir))):
                    ...
                    # Rimuovi i track persi prima di salvare le annotazioni
                    tracker.remove_lost_tracks(video_output_path)
