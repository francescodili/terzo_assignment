...
def extract_crops(pil_img, boxes, crops, encoder, det_path=None):
    ...
    if not crops:  # Inizializzazione
        ...
        for (xmin, ymin, xmax, ymax) in boxes.tolist():
            ...
            size = (xmax - xmin, ymax - ymin)
            crop = cv2.getRectSubPix(img, patchSize=(int(size[0]), int(size[1])), center=(center[0], center[1]))
            id = compare_crops(crop, crops, encoder, pil_img)
            crops[id] = [crop, (xmin, ymin, xmax, ymax)]
    
    return crops

def compare_crops(crop, crops, encoder, image):
    similarity = {}
    features_new_crop = encoder(image, [crop])[0]  # Estrarre le feature del nuovo crop
    
    for id, (img, bbox) in crops.items():
        # Calcola la similarità SSIM
        sim_ssim = similarity_between_crops(crop, img)
        
        # Estrai le feature dell'immagine esistente
        features_existing_crop = encoder(image, [img])[0]
        
        # Calcola la distanza tra le feature
        distance = np.linalg.norm(features_new_crop - features_existing_crop)
        
        # Combina la similarità SSIM e la distanza delle feature
        combined_score = (sim_ssim + (1 - distance)) / 2  # Peso uguale tra SSIM e distanza delle feature
        similarity[id] = combined_score
    ...
    if max_sim > 0.7:  # Soglia combinata
        return max_id
    else:  # Crea un nuovo id se la similarità è minore di 0.7
...
def main():
    ...
    encoder, _ = initialize_deepsort(model_path)
    crops = {}
        ...
        for video_dir in os.listdir(os.path.join(data_path, dir)):
            if video_dir == 'img1':
                for frame_path in os.listdir(os.path.join(data_path, dir, video_dir)):
                    ...
                    crops = extract_crops(img, bboxes_scaled, crops, encoder, det_path=det_path)
